...
test("必須機能とレスポンシブ設計をソースに備える", async () => {
  ...
  assert.match(app, /function saveTaskPatch/);
  assert.match(app, /function saveTaskDate/);
  assert.match(app, /className="detail-panel-resizer"/);
  assert.match(app, /role="separator"/);
  assert.match(app, /totonou-detail-panel-width/);
  assert.match(app, /className="detail-title-input"/);
  assert.match(app, /aria-label="リマインダー"/);
  assert.match(app, /className="detail-section detail-editor-stack detail-memo-editor"/);
  assert.match(app, /入力内容は自動保存されます/);
  assert.doesNotMatch(app, /className="task-row-menu"/);
  assert.doesNotMatch(app, /openEditForm\(selectedTask\)/);
  ...
  assert.match(css, /\.detail-panel-resizer/);
  assert.match(css, /--detail-panel-width/);
  assert.match(css, /\.detail-title-input/);
  assert.match(css, /\.detail-date-controls/);
  assert.match(css, /\.detail-memo-editor textarea/);
  assert.doesNotMatch(css, /\.task-row-menu/);
